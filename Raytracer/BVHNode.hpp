...
public:
		...
		:Renderable(nullptr), nodeDepth_(maxDepth)
	{
		...
		//     b. There's only one Renderable left in the list for this child.
		...
		:Renderable(nullptr)
	{
		...
		for (int f = 0; f < faceIndices->size(); ++f) {
			...
			sortedIndices.push_back(std::pair<float, int>(pos[splittingAxis], f));
		}

		std::sort(sortedIndices.begin(), sortedIndices.end(), [](const std::pair<float, int>& a, const std::pair<float, int>& b) { return a.first > b.first; });
		for (int f = 0; f < sortedIndices.size(); ++f) {
			if (f < sortedIndices.size() / 2)
				faces0.push_back((*faceIndices)[sortedIndices[f].second]);
			else
				faces1.push_back((*faceIndices)[sortedIndices[f].second]);
		}
